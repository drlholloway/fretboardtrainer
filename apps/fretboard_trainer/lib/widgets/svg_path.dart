...
/// Parses the subset of SVG path syntax used by the bundled glyphs:
/// M/m, L/l, H/h, V/v, C/c, S/s, Z/z with absolute and relative forms.
Path parseSvgPath(String d) {
  final path = Path();
  final tokens = RegExp(r'[MmLlHhVvCcSsZz]|-?\d*\.?\d+(?:e-?\d+)?')
      .allMatches(d)
      .map((m) => m.group(0)!)
      .toList();
  var i = 0;
  var cmd = '';
  var x = 0.0, y = 0.0, sx = 0.0, sy = 0.0, cx = 0.0, cy = 0.0;
  double num() => double.parse(tokens[i++]);
  bool more() => i < tokens.length && !RegExp(r'[A-Za-z]').hasMatch(tokens[i]);
  ...
  while (i < tokens.length) {
    ...
    switch (cmd.toUpperCase()) {
      ...
      case 'C':
        ...
        y = ny;
      case 'S':
        var x2 = num(), y2 = num(), nx = num(), ny = num();
        if (rel) {
          x2 += x;
          y2 += y;
          nx += x;
        ...
        y = ny;
      default:
        throw FormatException('unsupported path command $cmd');
    }
...
/// Treble clef outline, public domain (Wikimedia Commons `GClef.svg`).
/// Viewbox 15.186 × 40.768; the G line crosses the curl at about y = 27.7.
...
const trebleClefWidth = 15.186;
const trebleClefHeight = 40.768;

/// Where the G4 line crosses the clef, as a fraction of its height.
const trebleClefGLine = 0.68;
